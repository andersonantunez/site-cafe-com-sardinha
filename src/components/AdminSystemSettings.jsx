...
export default function AdminSystemSettings({ onStatus }) {
  ...
  const table = useAdminTable(items, { searchFields: ['chave', 'valor', 'descricao'], initialSort: 'chave' })
  ...
  return <div><div className="admin-heading"><div><span className="eyebrow">Tempo de execução</span><h1>Configurações do Sistema</h1><p>Variáveis globais aplicadas sem recompilar a aplicação.</p></div><button onClick={() => setForm(empty)}><Plus/> Nova variável</button></div>
    ...
    <AdminTableToolbar table={table} placeholder="Pesquisar chave, valor ou descrição"/><div className="admin-table-wrap"><table><thead><tr><SortableHeader table={table} sortKey="chave">Chave</SortableHeader><SortableHeader table={table} sortKey="valor">Valor</SortableHeader><SortableHeader table={table} sortKey="descricao">Descrição</SortableHeader><th>Ações</th></tr></thead><tbody>{table.rows.map(item => <tr key={item.chave}><td><strong>{item.chave}</strong></td><td>{item.valor}</td><td>{item.descricao}</td><td><div className="admin-row-actions"><button aria-label="Editar" onClick={() => setForm({ ...item, originalKey: item.chave })}><Edit3/></button>{item.chave !== 'carteira_cafe_usuario_email' && <button aria-label="Excluir" className="danger" onClick={() => remove(item)}><Trash2/></button>}</div></td></tr>)}</tbody></table></div><AdminTablePagination table={table}/>
  </div>
}
