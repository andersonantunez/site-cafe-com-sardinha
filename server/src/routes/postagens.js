...
postagensRouter.get('/', async (req, res) => {
  const { rows } = await query('SELECT id, titulo, conteudo, hashtags, url, publico, data_publicacao, criado_em, atualizado_em FROM postagens WHERE publico = TRUE ORDER BY data_publicacao DESC NULLS LAST, id DESC')
  res.json(rows)
})
...
postagensRouter.post('/', async (req, res) => {
  const { titulo, conteudo = '', hashtags = [], url, publico = true, data_publicacao = null } = req.body
  if (!titulo?.trim() || !url?.trim()) return res.status(400).json({ erro: 'Título e URL são obrigatórios.' })
...
postagensRouter.put('/:id', async (req, res) => {
  const { titulo, conteudo = '', hashtags = [], url, publico = true, data_publicacao = null } = req.body
  if (!titulo?.trim() || !url?.trim()) return res.status(400).json({ erro: 'Título e URL são obrigatórios.' })
...
postagensRouter.delete('/:id', async (req, res) => {
